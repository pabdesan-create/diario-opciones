...
export default function App() {
  ...
  const [hkdUsd, setHkdUsd] = useState(() => parseFloat(LS.get('fx-hkdusd') || '7.83')) // HKD por 1 USD
  ...
  // Tipos de cambio en vivo (EUR, GBP, HKD respecto a USD) al abrir la app.
  // Si falla la red, se queda con el valor guardado/por defecto sin romper nada.
  useEffect(() => {
    fetch('https://api.frankfurter.dev/v1/latest?base=USD&symbols=EUR,GBP,HKD')
      .then(r => r.json())
      .then(d => {
        const r = d?.rates
        if (!r) return
        if (r.EUR) { const eur = parseFloat((1 / r.EUR).toFixed(4)); setEurUsd(eur); LS.set('eur-usd', eur) }
        if (r.GBP) { const gbp = parseFloat((1 / r.GBP).toFixed(4)); setGbpUsd(gbp); LS.set('gbp-usd', gbp) }
        if (r.HKD) { setHkdUsd(r.HKD); LS.set('fx-hkdusd', r.HKD) }
      })
      .catch(() => {})
  }, [])
...
MONEDA: $=USD, €=EUR, £=GBP, HK$ o mercado SEHK=HKD
contratos = número tras Vendido/Comprado/Assigned/Expired

MULTIPLICADOR (nº de acciones por contrato) — crítico para Hong Kong (SEHK), NUNCA asumas 100 ahí:
- Si junto al strike aparece un número entre paréntesis, p.ej. "67.5 Put (200)" → ESE es el multiplicador, úsalo tal cual
- Si no aparece el paréntesis, CALCÚLALO: multiplicador = (importe_total ÷ precio_por_acción) ÷ contratos
  Ejemplo: SEHK, Vendido 1, precio 13.8 HKD, total 1.380 HKD → 1380÷13.8=100 acciones ÷ 1 contrato = multiplicador 100
  Ejemplo: SEHK, "67.5 Put (200)", Vendido 4, precio 1.02 HKD, total 816 HKD → paréntesis ya dice 200 (verificación: 816÷1.02=800÷4=200 ✓)
- Para mercados no-HK (USA, Europa) el multiplicador es 100 salvo que el extracto muestre otro valor explícito
...
[{"tipo":"APERTURA o CIERRE","estrategia":"VPUT/VCALL/CPUT/CCALL","ticker":"","fecha":"YYYY-MM-DD","vencimiento":"YYYY-MM-DD","strike":0,"contratos":1,"multiplicador":100,"prima":0,"precio_cierre":0,"beneficio":0,"moneda":"USD","assigned":false,"notas":""}]` },
...
MULTIPLICADOR (nº de acciones por contrato) — crítico para Hong Kong (SEHK), NUNCA asumas 100 ahí:
- Si junto al strike aparece un número entre paréntesis, p.ej. "67.5 Put (200)" → ESE es el multiplicador, úsalo tal cual
- Si no aparece, CALCÚLALO: multiplicador = (importe_total ÷ precio_por_acción) ÷ contratos
- Para mercados no-HK (USA, Europa) el multiplicador es 100 salvo que el extracto muestre otro valor explícito
    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'system-ui,-apple-system,sans-serif', color: C.txt }}>
      ...
      {showCfg && (
        ...
        <div style={{ background: '#060c18', borderBottom: `1px solid ${C.brd}`, padding: '12px 20px', display: 'flex', gap: 12, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <div>
            <label style={{ fontSize: 10, color: C.dim, fontWeight: 700, textTransform: 'uppercase', display: 'block', marginBottom: 4 }}>USD/HKD</label>
            <input type="number" step="0.01" value={hkdUsd} onChange={e => setHkdUsd(parseFloat(e.target.value) || 7.83)}
              style={{ width: 80, background: C.bg, border: `1px solid ${C.brd}`, color: C.gold, borderRadius: 6, padding: '8px 10px', fontSize: 12, outline: 'none' }} />
          </div>
          <button onClick={() => { LS.set('ib-api-key', apiKey); LS.set('eur-usd', eurUsd); LS.set('gbp-usd', gbpUsd); LS.set('fx-hkdusd', hkdUsd); setShowCfg(false) }}
